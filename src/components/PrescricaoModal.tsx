...
const vias = ["Oral", "Endovenosa", "Intramuscular", "Subcutânea", "Tópica", "Inalatória"];
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth scroll="paper">
        ...
        <Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
          ...
          <Box>
            ...
            <Typography variant="caption" color="text.secondary">
              Paciente: {chamado.paciente.nome} · {chamado.paciente.idade} anos
            </Typography>
          </Box>
        </Stack>
      ...
      <DialogContent dividers>
        ...
        <Box sx={{ p: 2, border: "1px dashed", borderColor: "divider", borderRadius: 2, mb: 2 }}>
          <Grid container spacing={1.5}>
            <Grid size={{ xs: 12, md: 4 }}>
              <TextField
                fullWidth size="small" label="Medicamento" placeholder="Ex: Paracetamol"
                value={novoMed.nome} onChange={(e) => setNovoMed({ ...novoMed, nome: e.target.value })}
              />
            </Grid>
            <Grid size={{ xs: 6, md: 2 }}>
              <TextField
                fullWidth size="small" label="Dose" placeholder="500mg"
                value={novoMed.dose} onChange={(e) => setNovoMed({ ...novoMed, dose: e.target.value })}
              />
            </Grid>
            <Grid size={{ xs: 6, md: 2 }}>
              <TextField
                select fullWidth size="small" label="Via"
                value={novoMed.via} onChange={(e) => setNovoMed({ ...novoMed, via: e.target.value })}
              >
                {vias.map((v) => (
            ...
            <Grid size={{ xs: 6, md: 2 }}>
              <TextField
                fullWidth size="small" label="Frequência" placeholder="6/6h"
                value={novoMed.frequencia} onChange={(e) => setNovoMed({ ...novoMed, frequencia: e.target.value })}
              />
            </Grid>
            <Grid size={{ xs: 6, md: 2 }}>
              <TextField
                fullWidth size="small" label="Duração" placeholder="5 dias"
                value={novoMed.duracao} onChange={(e) => setNovoMed({ ...novoMed, duracao: e.target.value })}
              />
            </Grid>
          </Grid>
        ...
        {medicamentos.length > 0 && (
          <List dense sx={{ mb: 2, border: "1px solid", borderColor: "divider", borderRadius: 2 }}>
            {medicamentos.map((m, i) => (
              <ListItem key={i} divider={i < medicamentos.length - 1}>
                <ListItemText
                  primary={<Typography variant="body2" sx={{ fontWeight: 600 }}>{m.nome} {m.dose}</Typography>}
                  secondary={`${m.via} · ${m.frequencia || "-"} · ${m.duracao || "-"}`}
                />
                <ListItemSecondaryAction>
                  <IconButton edge="end" size="small" onClick={() => removerMedicamento(i)}>
        ...
        <Stack direction="row" spacing={1} sx={{ mb: 1 }}>
          <TextField
            fullWidth size="small" placeholder="Digite ou selecione um exame"
            value={novoExame} onChange={(e) => setNovoExame(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); adicionarExame(); } }}
          />
          <Button variant="outlined" startIcon={<AddIcon />} onClick={() => adicionarExame()}>
            Add
          </Button>
        </Stack>
        ...
        <Stack direction="row" spacing={0.5} sx={{ flexWrap: "wrap", gap: 0.5, mb: 1 }}>
          {examesSugeridos.map((e) => (
            <Chip
              ...
              onClick={() => adicionarExame(e)} disabled={exames.includes(e)}
            />
          ))}
        </Stack>
        ...
        {exames.length > 0 && (
          <Stack direction="row" spacing={0.5} sx={{ flexWrap: "wrap", gap: 0.5, mb: 2 }}>
            {exames.map((e) => (
              <Chip key={e} label={e} color="info" onDelete={() => removerExame(e)} />
            ))}
          </Stack>
        )}
        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 8 }}>
            <TextField
              label="Orientações ao paciente" multiline minRows={3} fullWidth size="small"
              placeholder="Repouso, hidratação, sinais de alerta..."
              value={orientacoes} onChange={(e) => setOrientacoes(e.target.value)}
            />
          </Grid>
          <Grid size={{ xs: 12, md: 4 }}>
            <TextField
              label="Retorno" fullWidth size="small" placeholder="Em 7 dias"
